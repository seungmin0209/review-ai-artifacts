...
2-1. **여러 세션이 동시에 쓴다 — 남의 문서를 건드리지 않는다.** 서버는 자기를 띄운 세션 ID 를 `owner` 로 들고 있고(`/health`), 제출 이벤트에도 `owner` 가 실린다.
   - **주인이 남이라고 해서 곧바로 물러서지 않는다 — 살아 있는지부터 본다.** `/health` 는 `owner` 와 함께 `watched` 를 준다.
     `owner` 가 남인데 `watched: false` 면 그 세션은 끊긴 것이다. **이때 물러서면 사용자는 버튼이 전부 회색인 죽은 화면을 본다.**
     사용자가 "열어 줘" 라고 할 때 회색 화면을 원하는 경우는 없다. 확인을 기다리지 말고 **곧바로 heartbeat 를 보내 이어받는다.**
     heartbeat 는 듣기 시작하는 것일 뿐 남의 것을 빼앗는 동작이 아니므로 `--close` 도 재시작도 필요 없다.
     (2026-09-21 실제 사고 — 9월 19일에 끝난 세션이 주인인 서버를 그대로 열어 주고 감시를 걸지 않아, 사용자가 회색 화면에서 [마무리] 를 눌렀고 그 제출이 아무에게도 가지 않았다.)
   - `owner` 가 남이면서 `watched: true` 일 때만 **살아 있는 남의 문서**다. 서버를 재시작·`--close` 하지 않고, 그 문서의 `status: new` 이벤트도 처리하지 않는다. 훅도 이 경우 띄우지 않고 알리기만 한다.
     이때도 물러서는 대신 **heartbeat 를 함께 보내면 된다** — 둘 다 명단에 올라 사용자가 선택칸에서 고른다(아래 fork 항목).
   - 살아 있는 세션에게서 **단독으로 빼앗아야** 할 때만 사용자에게 먼저 확인한다. 확인 없이 포트를 빼앗으면 사용자의 제출이 엉뚱한 세션으로 간다.
   - **문서를 띄웠거나 이미 떠 있는 것을 열어 줬으면, 같은 턴에 반드시 Monitor 를 건다.** 새로 띄운 경우든 재사용한 경우든 예외가 없다. 감시 없이 넘긴 화면은 사용자에게 고장으로 보인다.
...
| 좌클릭 드래그 (빈 여백에서 시작) | **박스 선택.** 상자에 걸친 요소가 모두 잡힌다. 글자 위에서 시작하면 지금까지처럼 글자 선택이다 |
| Shift+클릭 | 그 요소를 선택에 **더하거나 뺀다.** 상태줄에 "N곳 선택됨" 이 뜬다. Esc 로 전부 해제 |
| 여러 요소에 걸쳐 드래그한 뒤 우클릭 | 걸친 요소들이 자동으로 선택된다. 예전에는 한 요소로만 달렸다 |
| 선택이 있는 상태에서 우클릭 | 고른 곳 **전부에 댓글 하나**를 단다. 이벤트에 `paths` 배열이 실린다 |
